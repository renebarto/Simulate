...
struct Nodes
{
    ...
    static ORGNode::Ptr CreateORGNode(std::wstring const & value, Location const & location)
    {
        return std::make_shared<ORGNode>(value, location);
    }
    static ENDNode::Ptr CreateENDNode(std::wstring const & value, Location const & location)
    {
        return std::make_shared<ENDNode>(value, location);
    }
    template<class SegmentType, class AddressType>

...
bool IsEqual(const std::wstring & lhs, const std::wstring & rhs)
{
    return (lhs == rhs);
}

bool IsEqualIgnoreCase(const std::wstring & lhs, const std::wstring & rhs)
{
    if (lhs.size() != rhs.size())
        return false;
    ...
    size_t size = rhs.size();
    for (size_t n = 0; n < size; n++)
        if (towlower(lhs[n]) != towlower(rhs[n]))
            return false;
...
{
    if ((text.length() >= 2) && (text[0] == L'\"') && (text[text.length() - 1] == L'\"'))
    {
        return text.substr(1, text.length() - 2);
    }
    return text;
...
std::wstring TrimStart(const std::wstring & text)
{
    size_t firstNonSpace = 0;
    while (iswspace(text[firstNonSpace]))
        firstNonSpace++;
    return text.substr(firstNonSpace);
}

std::wstring TrimEnd(const std::wstring & text)
{
    size_t lastNonSpace = text.length();
    while (iswspace(text[lastNonSpace - 1]))
        lastNonSpace--;
    return text.substr(0, lastNonSpace);
}

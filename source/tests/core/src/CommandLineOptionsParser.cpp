...
CommandLineOptionsParser::CommandLineOptionsParser() :
    ...
    xmlOutput()
{
    Core::CommandLineOptionGroupPtr group = std::make_shared<Core::CommandLineOptionGroup>("Test runner", "Runs tests");
    group->AddOptionRequiredArgument("runsuite", 's',
                                     "Run only specified test suite (default = all)", &testSuiteName);
    group->AddOptionRequiredArgument("runfixture", 'f',
                                     "Run only specified test fixture (default = all)", &testFixtureName);
